...
- `-i` (interactive) - зберігає STDIN відкритим для введення команд
- `-t` (tty) - виділяє псевдо-термінал для правильного відображення
- `--rm` - автоматично видаляє контейнер після завершення роботи
- `-v $(pwd)/data:/app/data` - монтує локальну папку `data` для збереження даних

**Що відбувається:**
- Файл `transactions.json` зберігається в папці `data` на вашому комп'ютері
- Дані не втрачаються при перезапуску контейнера
- Папка `data` створюється автоматично при першому запуску

### Docker Compose (альтернативний спосіб)
Для використання Docker Compose з інтерактивним додатком:

```bash
# Збудувати та запустити у фоновому режимі
docker compose up -d

# Підключитися до запущеного контейнера
docker compose exec accounting-app /bin/bash
# Або одразу запустити додаток
docker compose exec accounting-app dotnet AccountingCLI.dll
```

**Або простіше - використовуйте прямий запуск:**
```bash
docker run -it --rm -v $(pwd)/data:/app/data accounting-app
```

**Примітка:** Для інтерактивних консольних додатків краще використовувати `docker run` замість `docker compose up`.

### 📤 Публікація на Docker Hub

Ви можете опублікувати образ на Docker Hub для легкого доступу:

```bash
# 1. Увійти в Docker Hub
docker login

# 2. Створити тег з вашим username
docker tag accounting-app yourusername/accounting-app:latest

# 3. Завантажити на Docker Hub
docker push yourusername/accounting-app:latest
```

**Після публікації інші можуть використовувати ваш образ:**
```bash
# Завантажити та запустити
docker run -it --rm -v $(pwd)/data:/app/data yourusername/accounting-app:latest
```

**Або використовуйте готовий образ з Docker Hub:**
```bash
...
Можна налаштувати GitHub Actions для автоматичної публікації при push до репозиторію.
